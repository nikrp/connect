...
export default function Requests() {
  ...
  const calculateMatchScore = (request: Request, userProfile: any): number => {
    ...
    if (userSkills.length > 0 && requestTags.length > 0) {
      const userSkillSlugs = userSkills.map(
        (s: any) => s.slug?.toLowerCase() || "",
      );
      const requestTagSlugs = requestTags.map(
        (t: any) => t.slug?.toLowerCase() || "",
      );
      const matchingTags = requestTagSlugs.filter((tag: string) =>
        userSkillSlugs.includes(tag),
      );
      const tagScore =
        (matchingTags.length / Math.max(requestTagSlugs.length, 1)) * 100;
      score += tagScore * 0.5;
      totalWeight += 0.5;
    }
    ...
    if (userInterests.length > 0 && requestTags.length > 0) {
      const userInterestSlugs = userInterests.map(
        (i: any) => i.slug?.toLowerCase() || "",
      );
      const requestTagSlugs = requestTags.map(
        (t: any) => t.slug?.toLowerCase() || "",
      );
      const matchingInterests = requestTagSlugs.filter((tag: string) =>
        userInterestSlugs.includes(tag),
      );
      const interestScore =
        (matchingInterests.length / Math.max(requestTagSlugs.length, 1)) * 100;
    ...
    if (userSkills.length > 0 && requestTags.length > 0) {
      ...
      const exactMatches = requestTagSlugs.filter((tag: string) =>
        userSkillSlugs.includes(tag),
      ).length;
      const bonusScore = Math.min(
        (exactMatches / userSkills.length) * 100,
        100,
      );
  ...
  const fetchRequests = async (currentUserId?: string) => {
    try {
      ...
      let query = supabase
        ...
        .select(
          "*, creator_profile:profiles(id, name, profile_photo, pronouns)",
        )
      ...
      toast.error("Error fetching requests:", {
        description: (error as any)?.message || "Unexpected error",
      });
    } finally {
      setLoading(false);
    }
    ...
    const matchesTags =
      ...
      selected.some(
        (tag) =>
          ...
          request.tags.some((t: any) => t.slug === tag),
      );
    ...
    <div className={``}>
      ...
      <div className={`flex items-center flex-wrap gap-1.5 w-full mb-2.5`}>
        <Input
          ...
          className={`w-full lg:w-3/6 xl:w-5/12 text-sm`}
        />
        <Popover>
        ...
        <p className={`text-foreground/75 text-sm ml-1`}>
          {selected.length} Filters Applied
        </p>
        <div className={`h-5 mx-1.5 w-px rounded-full bg-foreground/15`} />
        <p className={`text-foreground/75 text-sm`}>
          {filteredRequests.length} of {requests.length} Results
        </p>
      </div>
      <div className={`grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3`}>
        {filteredRequests.length === 0 ? (
          <div
            className={`col-span-full flex items-center justify-center h-32`}
          >
...
function RequestCard({
  ...
  useEffect(() => {
    ...
    const checkExisting = async () => {
      ...
      try {
        ...
        if (error) {
          console.debug(
            "Error checking existing collab_members:",
            error.message,
          );
          return;
        }
  ...
  const requestToJoin = async () => {
    ...
    try {
      ...
      // Send notification to the collab creator
      ...
      window.dispatchEvent(
        new CustomEvent("collab-join-request", {
          detail: { collabId: request.id, userId: user.id },
        }),
      );
      setJoinOpen(false);
      setMessage("");
      ...
      toast.error("Error requesting to join", {
  ...
  return (
    <div className={`border rounded-lg p-3.5`}>
      <div className={`flex gap-5`}>
        <Link
          href={`/requests/posts/${request.creator_id}`}
        ...
        >
          ...
          <div className={`flex flex-col`}>
            <p
              className={`text-lg font-semibold text-foreground group-hover:underline`}
            >
          ...
          <Dialog open={joinOpen} onOpenChange={setJoinOpen}>
            ...
            <DialogContent>
              ...
              <p className={`text-sm text-foreground/80 mt-2`}>
                Send a short message to the creator along with your request
                (optional).
              </p>
              <div className="mt-4">
                <Textarea
