...
export default function Posts() {
  ...
  const collectCollabRequests = useCallback(async () => {
    try {
      // Get all collabs with members
      const { data: collabs, error: collabsError } = await supabase.rpc(
        ...
        new Set(
          collabs.flatMap((collab: any) =>
            Array.isArray(collab.members)
              ? collab.members.map((m: any) => m.user_id)
              : [],
          ),
        ),
      );
