...
export default function Profile() {
  ...
  const defaultInterests = Array.isArray(profile.interests)
    ? profile.interests
    : [];
  const defaultEmails = Array.isArray(profile.contact?.emails)
    ? profile.contact.emails
    : [];
  const defaultPhones = Array.isArray(profile.contact?.phones)
  ...
  const [photoCrop, setPhotoCrop] = useState<{ x: number; y: number }>({
    x: 0,
    y: 0,
  });
  ...
  const handlePhotoDialogChange = useCallback(
    (nextOpen: boolean) => {
  ...
  const handleCropComplete = useCallback(
    (_: CropArea, areaPixels: CropArea) => {
    ...
    try {
      ...
      if (removePhoto) {
        if (existingStoragePath) {
          ...
          if (removeError) {
            console.warn(
              "Unable to remove existing profile photo",
              removeError,
            );
          }
        }
      ...
      if (existingStoragePath && existingStoragePath !== filePath) {
        ...
        if (removeError) {
          console.warn(
            "Unable to clean up previous profile photo",
            removeError,
          );
        }
      }
  ...
  const onSubmit = async (vals: EditProfileFormValues) => {
    try {
      ...
      const emails = String(vals.emailsText || "")
        .split(",")
        .map((s: string) => s.trim())
        .filter(Boolean);
      const phones = String(vals.phonesText || "")
        .split(",")
        .map((s: string) => s.trim())
        .filter(Boolean);
      const contact = {
      ...
      const normalizedPhoto = vals.profile_photo?.trim()
        ? vals.profile_photo.trim()
        : null;
  ...
  return (
    <div className={`bg-card rounded-xl w-11/12 md:w-9/12 mx-auto mb-5`}>
      <div
        className={`p-10 md:p-10 bg-muted text-card-foreground rounded-t-xl`}
      >
        <div className={`flex flex-col md:flex-row md:justify-between`}>
          <div className={`flex flex-row items-start gap-4 mb-5`}>
            ...
            >
              <DialogTrigger asChild>
                ...
                >
                  <Avatar
                    className={`size-20 md:size-40 bg-sidebar p-2 rounded-full transition-transform group-hover:scale-[1.02]`}
                  >
              ...
              <DialogContent className="sm:max-w-[720px] p-6">
                <DialogHeader>
                  ...
                  <DialogDescription>
                    Upload, crop, and preview a new avatar before saving.
                  </DialogDescription>
                </DialogHeader>
                <div className="grid gap-6 md:grid-cols-[2fr_1fr]">
                      ...
                      {photoPreview && !removePhoto ? (
                        ...
                        <div className="flex h-full items-center justify-center text-sm text-muted-foreground px-6 text-center">
                          {removePhoto
                            ...
                            : "Drop a new image or paste a link to begin."}
                        </div>
                      )}
                    </div>
                    {photoPreview && !removePhoto && (
                      <div className="space-y-2">
                        ...
                        <input
                          ...
                          value={photoZoom}
                          onChange={(event) =>
                            setPhotoZoom(parseFloat(event.target.value))
                          }
                  ...
                  <div className="space-y-5">
                    <div className="space-y-2">
                      ...
                      <AvatarDropzone
                        value={selectedPhotoFile || photoPreview || undefined}
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <p className="text-xs text-muted-foreground">
                          This is how your profile will appear across Connect.
                        </p>
                      </div>
                    </div>
                    <div className="space-y-2">
                ...
                <DialogFooter>
                  <Button
                    type="button"
                    variant="outline"
                    onClick={() => handlePhotoDialogChange(false)}
                    disabled={savingPhoto}
                  >
                    Cancel
                  </Button>
            ...
            <div className={``}>
              ...
              <p className={`text-md md:text-xl font-sans text-foreground/80`}>
          ...
          <div className={`flex flex-col gap-2`}>
            <p className={`text-base flex items-center gap-2`}>
              <Building className={`text-foreground/75`} />
              {(profile.school || "")
                .split(" ")
                .map(
                  (word: string) =>
            ...
            <p className={`text-base flex items-center gap-2`}>
              <Timer className={`text-foreground/75`} />
              {Array.isArray(profile.preferredWorkTimes)
                ? profile.preferredWorkTimes.join(", ")
                : ""}
            </p>
          </div>
        </div>
        ...
        {profile.contact && (
          <div className="mb-5">
            ...
            <div className="flex flex-col gap-2">
              {/* Emails */}
              {Array.isArray(profile.contact.emails) &&
                profile.contact.emails.length > 0 && (
                  <div className="flex flex-wrap gap-2">
                    {profile.contact.emails.map(
              ...
              {/* Phones */}
              {Array.isArray(profile.contact.phones) &&
                profile.contact.phones.length > 0 && (
                  <div className="flex flex-wrap gap-2">
                    {profile.contact.phones.map(
        ...
        <div className={`flex items-center gap-1.5 flex-wrap`}>
          {(Array.isArray(profile.skills) ? profile.skills : []).map(
            (
              skill: { slug: string; label: string; custom: boolean },
              index: number,
            ) => {
              return (
                <p className={`px-3 py-1 rounded-full bg-input`} key={index}>
                  {skill.label}
                </p>
              );
            },
        ...
        <div className={`flex items-center gap-1.5 flex-wrap`}>
          {(Array.isArray(profile.interests) ? profile.interests : []).map(
            (
              skill: { slug: string; label: string; custom: boolean },
              index: number,
            ) => {
      ...
      <div
        className={`p-10 md:p-10 bg-input text-input-foreground dark:bg-card dark:text-card-foreground rounded-b-xl`}
      >
        ...
        {collabsLoading ? (
          <p className={`w-full text-center mt-10 text-foreground/75`}>
            Loading collabs...
          </p>
        ) : collabs.length === 0 ? (
          ...
          <div className={`grid grid-cols-1 md:grid-cols-2 gap-3 mt-6`}>
            {collabs.map((request: any) => {
              const postDate = new Date(request.created_at).toLocaleString(
                "en-US",
                {
              ...
              return (
                <div
                  key={request.id}
                  className={`border rounded-lg p-3.5 bg-background`}
                >
                  <div className={`flex gap-5`}>
                    <img
                      src={
                        request.creator_profile?.profile_photo ||
                    ...
                    <div className={`flex flex-col`}>
                      <p className={`text-lg font-semibold text-foreground`}>
                        ...
                        <span className={`text-sm text-gray-500 font-normal`}>
                          {request.creator_profile?.pronouns
                            ? ` (${request.creator_profile.pronouns})`
                            : ""}
                        </span>
                      </p>
                  ...
                  <p className={`text-foreground/80 line-clamp-3 text-sm mb-2`}>
                    {request.description}
                  </p>
                  <div className={`mb-3.5 flex flex-wrap item-center gap-2.5`}>
                    {(Array.isArray(request.tags) ? request.tags : []).map(
                      (tag: any, index2: number) => {
                        return (
                          <p
                  ...
                  <div className={`grid grid-cols-1 gap-3.5`}>
                    <Dialog>
                      ...
                      <DialogContent>
                        ...
                        <div
                          className={`mb-3.5 flex flex-wrap item-center gap-2.5`}
                        >
                          {(Array.isArray(request.tags)
                            ? request.tags
                            : []
                          ).map((tag: any, index2: number) => {
                            return (
                              <p
                                onClick={() => setOpen(true)}
                                key={index2}
