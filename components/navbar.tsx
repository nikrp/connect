...
export default function Navbar() {
  ...
  useEffect(() => {
    async function collectProfilePhoto() {
      ...
      if (sError) {
        console.error(
          "Error downloading profile photo from Supabase storage:",
          sError,
        );
      } else {
  ...
  return (
    <div className={`w-full py-3.5 pb-5 flex items-center gap-5 mb-2`}>
      ...
      <Link
        href="/?noRedirect=1"
        className={`text-2xl font-semibold font-sans text-foreground mr-auto`}
      >
        Connect
      </Link>

      <div className={`flex items-center gap-3.5`}>
        {user?.id && <NotificationsPopover userId={user.id} />}
        <DropdownMenu>
          ...
          <DropdownMenuContent>
            <DropdownMenuGroup>
              <DropdownMenuLabel className={`text-sm text-foreground/60`}>
                Themes
              </DropdownMenuLabel>
              <DropdownMenuItem
                className={`cursor-pointer`}
        ...
        <DropdownMenu>
          <DropdownMenuTrigger
            asChild
            className={`cursor-pointer hover:bg-foreground/10 transition-all p-2 ${isMobile ? `rounded-full` : `rounded-lg`}`}
          >
